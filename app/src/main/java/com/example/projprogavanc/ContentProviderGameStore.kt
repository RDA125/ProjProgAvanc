...
class ContentProviderGameStore: ContentProvider() {
    ...
    override fun query(
        ...
        val cursor = when(getUriMatcher().match(uri)){
            ...
            URI_GAME_SPECIFIC -> TDBGames(db).query(columns, "${TDBGames.C_ID} = ?", arrayOf("${id}"), null, null, null)
            URI_STORE_SPECIFIC -> TDBStores(db).query(columns, "${TDBStores.C_ID} = ?", arrayOf("${id}"), null, null, null)
            URI_GAME_STORE_SPECIFIC -> TDBGame_Store(db).query(columns, "${TDBGame_Store.C_ID}= ?", arrayOf("${id}"), null, null, null)
    ...
    override fun delete(uri: Uri, selection: String?, selectionArgs: Array<out String>?): Int {
        ...
        val DeletedEntries = when(getUriMatcher().match(uri)){
            ...
            URI_GAME_STORE_SPECIFIC -> TDBGame_Store(db).delete("${BaseColumns._ID} = ?", arrayOf("${id}"))
            else -> 0
        }
    ...
    companion object{
        ...
        fun getUriMatcher(): UriMatcher{
            ...
            uriMatcher.addURI(AUTHORITY, TDBGames.T_NAME, URI_GAMES)
            uriMatcher.addURI(AUTHORITY, "${TDBGames.T_NAME}/#", URI_GAME_SPECIFIC)
