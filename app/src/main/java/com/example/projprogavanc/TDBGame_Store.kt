...
class TDBGame_Store (db: SQLiteDatabase): TDB(db, T_NAME){
    ...
    override fun create(){
        ...
        db.execSQL("create table $T_NAME (${BaseColumns._ID} INTEGER primary key autoincrement, $C_GAME_ID Integer not null, $C_STORE_ID Integer not null, $C_PRECO Real not null, foreign key ($C_GAME_ID) references ${TDBGames.T_NAME} (${BaseColumns._ID}) on delete restrict, foreign key ($C_STORE_ID) references ${TDBStores.T_NAME} (${BaseColumns._ID}) on delete restrict) ")
    ...
    companion object{
        ...
        const val T_NAME = "GameStore"
        const val C_PRECO = "Preco"
        const val C_ID = "$T_NAME.${BaseColumns._ID}"
        const val C_GAME_ID = "Game_Id"
        const val C_STORE_ID = "Store_id"

        val ALL_COLUMNS = arrayOf(C_ID, C_GAME_ID, C_STORE_ID, C_PRECO, TDBGames.C_NAME, TDBGames.C_GAMETYPE_ID, TDBGameTypes.C_TYPE, TDBStores.C_NAME, TDBStores.C_ADDRESS, TDBStores.C_STORETYPE_ID, TDBStoreTypes.C_TYPE)
    }
}